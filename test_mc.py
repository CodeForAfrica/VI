...
if not api_key:
    ...
    try:
        ...
        result = mc.story_list(
            query=query,
            start_date=start_date, # Try string format first
            end_date=end_date,     # Try string format first
        ...
        if isinstance(result, tuple) and len(result) == 2:
            ...
            print(f"Result seems to be an iterable. Length: {len(result) if hasattr(result, '__len__') else 'N/A'}")
            if result:
