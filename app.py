...
# --- Monthly Navigation Logic (Dynamic Fetch) ---
...
def fetch_monthly_intelligence():
    ...
    # Fetch data for the selected month specifically
    with st.spinner(f"📡 Fetching live intelligence for {st.session_state.current_date.strftime('%B %Y')}..."):
        # Assuming your DataManager can take date ranges
...
with n1:
    if st.button("⬅️ Previous Month (Fetch November)"):
        st.session_state.current_date = (st.session_state.current_date - timedelta(days=1)).replace(day=1)
...
with n3:
    if st.button("Next Month (Fetch Future) ➡️"):
        st.session_state.current_date = (st.session_state.current_date + timedelta(days=32)).replace(day=1)
        fetch_monthly_intelligence()
        st.rerun()

# --- FETCH DATA FROM DB ---
# We fetch everything available, then filter by the selected month in the UI
raw_df = mgr.fetch_articles(limit=1000) 

if not raw_df.empty:
    raw_df['published_at'] = pd.to_datetime(raw_df['published_at'])
    target_month = st.session_state.current_date.month
    target_year = st.session_state.current_date.year
    
    # Filter view to current selected month
    df_view = raw_df[(raw_df['published_at'].dt.month == target_month) & 
                     (raw_df['published_at'].dt.year == target_year)]

    # --- Filter Card ---
    st.markdown('<div style="background:#1c2128; padding:15px; border-radius:10px; border:1px solid #444;">', unsafe_allow_html=True)
    ...
    with f4:
        ...
        if not df_view.empty:
            try:
                ...
                st.download_button("📥 PDF", data=pdf_bytes, file_name=f"Report_{target_month}_{target_year}.pdf")
            except: st.error("PDF Fail")
    st.markdown('</div>', unsafe_allow_html=True)

    st.write(f"**Articles Found:** {len(df_view)}")
    ...
    st.warning(f"No data in database for {st.session_state.current_date.strftime('%B %Y')}. Click 'Previous' to try fetching older data.")
